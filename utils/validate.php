				...
				foreach ($rules as $rule => $value) {
					...
					switch ($rule) {
						...
						case "unique":
							if (!is_null($this->conn)) {
								try {
									...
									$db_column = strtolower(preg_replace('/([a-z])([A-Z])/', '$1_$2', $key));

									$query = "SELECT id FROM {$value} WHERE {$db_column} = :{$db_column}";
									$stmt = $this->conn->prepare($query);
									$stmt->bindParam(":{$db_column}", $v);

									if ($stmt->execute()) {
										$count = $stmt->rowCount();
										if ($count > 0) {
											$this->errors[] = "El valor [$v] ya existe en la tabla {$value} -> {$db_column}.";
										}
									}
						...
						case "exist":
							if (!is_null($this->conn)) {
								try {
									// En lugar de usar la misma columna, siempre validamos contra 'id'
									...
									if ($stmt->execute()) {
										$count = $stmt->rowCount();
										if ($count == 0) {
											$this->errors[] = "El valor [$v] no existe en la tabla {$value}.";
										}
									}
								} catch (\PDOException $e) {
									$this->errors[] = "{$rule}: " . $e->getMessage();
